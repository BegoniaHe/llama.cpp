...
#pragma unroll
        for (int mask = 16; mask > 0; mask >>= 1) {
            amax = fmaxf(amax, __shfl_xor_sync(GGML_HIP_WARP_MASK, amax, mask, WARP_SIZE));
        }
        ...
        const float val0 = __shfl_sync(GGML_HIP_WARP_MASK, scaled_val, base, WARP_SIZE);
        const float val1 = __shfl_sync(GGML_HIP_WARP_MASK, scaled_val, base + 16, WARP_SIZE);
        const float val2 = __shfl_sync(GGML_HIP_WARP_MASK, scaled_val, base + 1, WARP_SIZE);
        const float val3 = __shfl_sync(GGML_HIP_WARP_MASK, scaled_val, base + 17, WARP_SIZE);
        ...
        const uint8_t q_lo_0 = __shfl_sync(GGML_HIP_WARP_MASK, q_val, base,      WARP_SIZE);
        const uint8_t q_lo_1 = __shfl_sync(GGML_HIP_WARP_MASK, q_val, base + 1,  WARP_SIZE);
        const uint8_t q_hi_0 = __shfl_sync(GGML_HIP_WARP_MASK, q_val, base + 16, WARP_SIZE);
        const uint8_t q_hi_1 = __shfl_sync(GGML_HIP_WARP_MASK, q_val, base + 17, WARP_SIZE);
